...
with `delay` + state-condition hacks), you describe each room **once**.
...
- If only `single_click_action` is set, it fires **immediately** on the press —
  no waiting for a possible double.
- If `long_press_action` is not set, the long-press timer is never armed.
...
**Manual:** copy `custom_components/button_actions/` into your HA `config/custom_components/`
...
Pick the trigger entity, the input mode/timing, and the target entities to
toggle for each gesture. Best for the common "toggle these lights" case.

### YAML (full power — scenes, scripts, templates, choose)
...
Copy the block for each room and change the entity ids. See
...
When `fire_events: true`, each gesture fires a `button_actions_gesture` event:
...
Use it to wire custom behavior with a single automation per device.
